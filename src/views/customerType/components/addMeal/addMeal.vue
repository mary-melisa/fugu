<template>
  <el-dialog class="commonDialog" :title="parentTitle === 1 ? '新增用户类型' : '编辑用户类型'" :visible.sync="visible" width="400px"
    :before-close="cancelModule">
    <el-form ref="addMeal" :rules="rules" class="commonForm mt45" :model="addMeal" label-width="35%">
      <el-form-item label="类型名称:" prop="categoryName" required>
        <el-input class="commonInput" v-model="addMeal.categoryName" maxlength="50"></el-input>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    // 添加
    mealSave () {
      const url = this.urlPrev + `api/bdusertype/addusertype`;
    ...
    // 编辑
    mealEdit () {
      const url = this.urlPrev + `api/bdusertype/editusertype`;
      axios({ method: 'post', url: url, data: this.addMeal })
        .then(rsp => {
          if (rsp.data.status == 1) {
            this.$emit('getParentTableData');
