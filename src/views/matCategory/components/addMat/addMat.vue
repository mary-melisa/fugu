<template>
  <el-dialog class="commonDialog" :title="parentTitle === 1 ? '新增物料类别' : '编辑物料类别'" :visible.sync="visible" width="400px"
...
export default {
  ...
  methods: {
    ...
    // 添加类别
    mealSave () {
      const url = this.urlPrev + `api/bdmateriatype/addmateriatype`;
    ...
    // 编辑类别
    mealEdit () {
      const url = this.urlPrev + `api/bdmateriatype/editmateriatype`;
      axios({ method: 'post', url: url, data: this.addMeal })
        .then(rsp => {
          if (rsp.data.status == 1) {
            this.$emit('getParentTableData');
