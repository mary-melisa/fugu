<template>
    <div class="foodCategory">
        <Breadcrumb :firstMenu="'备餐管理'" :secondMenu="'菜品类别'"/>
        <div class="search">
            <div class="fieldItem">
              <el-input placeholder="请输入关键字" class="commonInput" v-model="keyContent" style="width: 300px;"> </el-input>
            </div>
        ...
        <AddFood v-on:cancelModule="cancelModule" v-if="dialogVisible" v-on:getParentTableData="getTableData" :parentTitle="title" :parentCurrentMeal="currentMeal" v-on:setPageIndexDefault="setPageIndexDefault"></AddFood>
    </div>
</template>
...
import TableContent from '@/views/foodCategory/components/tableContent/tableContent.vue';
import AddFood from '@/views/foodCategory/components/addFood/addFood.vue';
export default {
    components:{
        Breadcrumb,
        AddFood,
    ...
    methods:{
      ...
      select(){
        ...
        getTableData(){
            if(this.keyContent){
                this.defaultProps.keyContent = this.keyContent;
            }else {
                if(this.defaultProps.keyContent) {
                    delete this.defaultProps.keyContent;
                }
            }
            const url= this.urlPrev + `api/bddishescategory/getdishescategory`;
      ...
      deleteMeal(rowObj) {
        ...
        const url = this.urlPrev + `api/bddishescategory/deldiscategory?ids=` + arr;
