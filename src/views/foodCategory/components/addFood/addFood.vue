<template>
  <el-dialog class="commonDialog" :title="parentTitle === 1 ? '新增菜品类别' : '编辑菜品类别'" :visible.sync="visible" width="400px"
...
export default {
  ...
  methods: {
    ...
    // 添加
    mealSave () {
      const url = this.urlPrev + `api/bddishescategory/adddiscategory`;
    ...
    // 编辑
    mealEdit () {
      const url = this.urlPrev + `api/bddishescategory/editdiscategory`;
      axios({ method: 'post', url: url, data: this.addMeal })
        .then(rsp => {
          if (rsp.data.status == 1) {
            this.$emit('getParentTableData');
